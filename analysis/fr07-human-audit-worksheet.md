...
No exact duplicates were found. Nine testcase rows participate in six partial-overlap relationships, each retaining an independent oracle, interaction, resource, or response-contract objective. All 35 Student Decision, 35 Student Reason, and 35 Student Correction Decision cells remain `PENDING HUMAN REVIEW`, for 105 pending student-owned cells. No final human decision is made here.
...
Oracle labels were normalized to `SUPPORTED`, `PARTIAL`, `NOT_SPECIFIED`, or `NOT_APPLICABLE` without strengthening the specification. Blocker effects were distinguished as setup-preventing versus oracle-limiting. Advisory recommendations do not populate student-owned decisions.
...
These values are AI evidence only and are not the student's final audit results.

## 11. Human Review Instructions

Review HIGH first, then MEDIUM, then LOW. For every worksheet row:

1. Choose exactly one Student Decision: `VALID`, `INVALID`, or `INCOMPLETE`.
2. Write a case-specific Student Reason grounded only in the authoritative inputs.
3. Choose exactly one Student Correction Decision: `NO_CHANGE`, `CORRECT`, `COMPLETE`, `RECLASSIFY_EXPLORATORY`, `REMOVE_DUPLICATE`, or `REMOVE_OUT_OF_SCOPE`.

Treat the AI recommendation as advisory. Confirm partial overlaps by checking whether each case has an independent oracle or objective. Do not infer HTTP status, schema, ownership, persistence, quantity, product, stock, or calculation rules from convention or runtime behavior.
